...
template <typename T>
T &moduloIndex(std::vector<T> &vector, const int32_t index) {
  int32_t adjustedIndex = index + vector.size();
  return vector[adjustedIndex % vector.size()];
}

template <typename T>
const T &moduloIndex(const std::vector<T> &vector, const int32_t index) {
  int32_t adjustedIndex = index + vector.size();
  return vector[adjustedIndex % vector.size()];
}

template <typename T>
std::pair<std::vector<T>, std::vector<T>> getSmallestClosedShape(
    const std::vector<T> &chain,
